...
export default function Page({ current, source, headings, propsTable, bundleSize, componentName }) {
  return (
    <>
      <NextSeo title={`JTUI - ${source.scope.title} | Components`} description={source.scope.description} />
      {source.scope.status == 'Coming soon' && <ComingSoon />}
      <header className="post-header">
        <Header css={{ opacity: source.scope.status == 'Coming soon' ? 0.5 : 1 }}>{source.scope.title}</Header>

        {(source.scope.status == 'Draft' || source.scope.status == 'Beta') && (
          <ComponentStatus type={source.scope.status} />
        )}

        {source.scope.description && <P className="description">{source.scope.description}</P>}
      ...
      <Article css={{ opacity: source.scope.status == 'Coming soon' ? 0.5 : 1 }}>
        <MDXRemote
          {...source}
...
export const getStaticProps = async ({ params }) => {
  ...
  const packageExists = await fetch(`https://registry.npmjs.org/@washingtonpost/wpds-${params.slug}`).then(
