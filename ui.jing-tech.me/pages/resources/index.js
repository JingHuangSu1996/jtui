...
import { Thumbnail, THUMBNAIL_SQUARE, THUMBNAIL_WIDE } from '~/components/Thumbnail';
...
const Page = ({ wrapper }) => (
  <>
    <NextSeo title={`JTUI - Resources`} />
    <Header as="h1">Resources</Header>
    <Description>
      Learn more about our workflow and how to use our tools in our guides. Watch our tutorials and workshops to
      discover the elegance and accessibility of WPDS.
    </Description>
    <TableofContents headings={wrapper.headings} css={{ margin: '$075 0' }} />
    {wrapper.content.categories.map(({ name, id, type, posts, size, description }) => (
      <React.Fragment key={name}>
        ...
        <SeeAllLink href={`/resources/${name.toLowerCase()}`} name={name} type={type} />
      </React.Fragment>
    ))}
  </>
);
...
export const getStaticProps = async () => {
  ...
  const recents = [...docs].sort((a, b) => new Date(b.data.publishDate) - new Date(a.data.publishDate)).slice(0, 3);
...
async function getWrapper(collections, recents) {
  ...
  const level = 2;
  const descriptions = {
    Guides: 'Explore the processes and tools we use in our step-by-step written guides.',
    Tutorials: 'Watch or read through our tutorials to understand key techniques and concepts.',
    Accessibility: 'Explore our accessibility checklist, testing strategies and considerations.',
    Workshops: 'Sharpen your design and development skills with our in-depth recorded workshops.',
