...
import { getNavigation, getAllPathsByCategory, getResource, getHeadings } from '~/services';
...
export default function Page({ current, source, headings }) {
  return (
    <>
      <NextSeo title={`JTUI - ${source.scope.title} | Resources`} description={source.scope.description} />
      <>
        <Breadcrumbs.Root>
          <Breadcrumbs.Item href="/resources">Resources</Breadcrumbs.Item>
          <Breadcrumbs.Item href={`/resources/${source.scope.kicker.toLowerCase()}`}>
            {source.scope.kicker}
          </Breadcrumbs.Item>
        </Breadcrumbs.Root>
        <header>
...
export const getStaticProps = async (response) => {
  const source = await getResource(`${SECTION}/${response.params.category}/${response.params.slug}`);
  const navigation = await getNavigation();
  const headings = await getHeadings(`${SECTION}/${response.params.category}/${response.params.slug}`);
