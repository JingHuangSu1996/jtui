...
import { Box, Icon, styled, Button, VisuallyHidden, theme } from '@washingtonpost/wpds-ui-kit';
    ...
    <>
      <Container as="header">
        ...
        >
          ...
          />
          <Button css={{ border: 'none' }} onClick={() => setMobileMenu(!isClosed)}>
      ...
      <List role="navigation" aria-label="Support and search">
        ...
        <ListItem>
          <Link href="/resources" passHref>
            <Anchor isCurrent={router.asPath.includes('/resources')}>Resources</Anchor>
          </Link>
        </ListItem>
        {/* <ListItem css={{ margin: '0 $100 0 0' }}>
          <Link href="/support" passHref>
            <Anchor isCurrent={router.asPath.includes('/support')}>Support</Anchor>
          </Link>
        </ListItem> */}
        <ListItem>{hideFromSmallScreen && <SearchForm />}</ListItem>
        <ListItem>
          <ThemeToggle css={{ position: 'fixed', marginTop: '-$100', zIndex: '$page' }} />
        </ListItem>
      </List>
    </>
  );
