...
const InputCheckbox = dynamic(() => import('./Components/Checkbox').then((mod) => mod.InputCheckbox));
...
const components = {
  ...
  Collapsible: ({ children, maxHeight }) => (
    <CollapsibleContainer maxHeight={maxHeight}>{children}</CollapsibleContainer>
  ),
  Alert: ({ position, variant, shadow, dismissable, css, children }) => (
    <AlertBanner.Root shadow={shadow} position={position} dismissable={dismissable} variant={variant}>
      <AlertBanner.Trigger />
      <AlertBanner.Content
        css={{
          '& a': {
