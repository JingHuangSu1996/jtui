...
const Preview = ({ isGuide, demoHeight }) => {
  ...
  return (
    <Box
      ref={iframeRef}
      as="iframe"
      src={`/playroom?code=${LZString.compressToEncodedURIComponent(firstRenderCode)}&isGuide=${isGuide}`}
      css={{
...
const OpenInPlayroom = ({ isGuide }) => {
  ...
  return (
    <Button
      ...
      density="compact"
      as="a"
      href={`/playroom?edit=1&code=${LZString.compressToEncodedURIComponent(code)}&isGuide=${isGuide}`}
      target="_blank"
...
const CustomSandpack = ({ isGuide, withPreview = false, hideNavBar = false, demoHeight, children }) => {
