...
function hexToRgbA(hex) {
  ...
  if (/^#([A-Fa-f0-9]{3}){1,2}$/.test(hex)) {
    ...
    c = '0x' + c.join('');
    let rgba = 'rgba(' + [(c >> 16) & 255, (c >> 8) & 255, c & 255].join(',') + ',1)';
...
function lookUpValue(rgba) {
  ...
  for (var token in Tokens.color.light) {
    if (Object.prototype.hasOwnProperty.call(Tokens.color.light[token], 'value')) {
...
export default function inlineSVG({ cushion, path, title, description, width, height }) {
  ...
  return (
    <SVGContainer cushion={cushion}>
      <ReactSVG
        ...
        loading={() => (
          <Image width={width} height={height} layout="fill" className={inlineSvgCss()} src={path} alt="" />
        )}
        beforeInjection={(svg) => {
