...
import { Button, Box, Icon, InputTextarea, theme } from '@washingtonpost/wpds-ui-kit';
...
const StyleConverter = () => {
  ...
  React.useEffect(() => {
    ...
    for (let i = 0; i < sll; i++) {
      ...
      if ((sheet.cssRules[0] && sheet.cssRules[0].cssText) === ':root { font-size: 16px; }') {
  ...
  function convertClassToEntry(className) {
    ...
    for (let i = 0; i < rll; i++) {
      ...
      if (rule.cssText.includes(`.${className} {`) || rule.cssText.includes(`.${className},`)) {
        const parts = rule.cssText.split(':');

        const prop = camelCase(parts[parts.length - 2].substring(parts[parts.length - 2].indexOf('{') + 1));

        const val = parts[parts.length - 1].substring(1, parts[parts.length - 1].indexOf(';'));
  ...
  function getToken(prop, val) {
    ...
    if (convertedVal.indexOf('px') !== -1 && convertedVal.indexOf('px') === convertedVal.length - 2) {
    ...
    if (convertedVal.includes('rgb(')) {
      convertedVal = `rgba(${convertedVal.substring(4, convertedVal.length - 1).replace(/ /g, '')},1)`;
    }
    ...
    switch (prop) {
      case 'border':
        ...
        return '`' + convertedVal.replace(rgba, '${theme.colors.' + tokenValue + '}') + '`';
      default:
